...
EXPORT(bool, sceAvPlayerGetAudioData, SceUID player_handle, SceAvPlayerFrameInfo *frame_info) {
    ...
    frame_info->stream_details.audio.size = static_cast<unsigned long>(player_info->player.last_channels * player_info->player.last_sample_count) * sizeof(int16_t);
...
EXPORT(uint32_t, sceAvPlayerGetStreamInfo, SceUID player_handle, SceUInt32 stream_no, SceAvPlayerStreamInfo *stream_info) {
    ...
    if (stream_no == 0) { // suspect always two streams: audio and video //first is video
        ...
        stream_info->stream_details.audio.size = static_cast<unsigned long>(player_info->player.last_channels * player_info->player.last_sample_count) * sizeof(int16_t);
