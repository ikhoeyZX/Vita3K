...
uint32_t Rack::get_required_memspace_size(MemState &mem, SceNgsRackDescription *description) {
    ...
    return sizeof(ngs::Rack) + description->voice_count * (sizeof(ngs::Voice) + buffer_size + static_cast<unsigned long>(description->patches_per_output * description->definition.get(mem)->output_count) * sizeof(ngs::Patch));
}
